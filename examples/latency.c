...
__set_it:
    ...
    printf("P_PERIOD_SIZE:%d\n",(int)p_psize);
    if (p_psize > (unsigned int)*bufsize)
        *bufsize = p_psize;
    snd_pcm_hw_params_get_period_size(c_params, &c_psize, NULL);
    printf("C_PERIOD_SIZE:%d\n",(int)c_psize);
...
int main(int argc, char *argv[])
{
    ...
    buffer = malloc((latency_min * snd_pcm_format_width(format) / 8) * 2);
    //setscheduler();
        ...
        */
        ...
        while (ok && frames_in < loop_limit) {
            ...
            if ((r = readbuf(chandle, buffer, latency, &frames_in, &in_max)) < 0)
                ...
                rt = writebuf(phandle, buffer, r, &frames_out);
                if(rt == 0)
                {
